...
|--------|---------|------------|
| `verify-setup.sh` | Pre-flight check: verify all components (Numaflow, DRANET, whereabouts, cert-manager, gpu-direct-comm, CoreDNS etcd) are deployed and operational. Auto-detects local/baremetal from kubectl context | — |
...
```bash
make verify-setup            # Pre-flight check (auto-detects environment)
make test-e2e-full-local     # Full-flow E2E on k3d
make test-e2e-full-baremetal  # Full-flow E2E on baremetal
```
